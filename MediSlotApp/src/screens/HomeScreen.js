...
import {
  View, Text, Image, Pressable, Dimensions, Animated,
  TouchableOpacity, ActivityIndicator, StyleSheet, StatusBar, Easing, Alert
} from "react-native";
...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      scrollRef.current?.scrollToIndex({ index: indexRef.current, animated: true });
    }, AUTOPLAY_MS);
    ...
    return (
      ...
      >
        ...
        <View style={s.cardBody}>
          ...
          {!!item.summary && <Text numberOfLines={2} style={s.cardSummary}>{item.summary}</Text>}
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[s.sectionHeader, { opacity: fade, transform: [{ translateY: slideY }] }]}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("HealthAwarenessList")}>
        ...
        <Animated.View style={{ opacity: fade, transform: [{ translateY: slideY }] }}>
          ...
          <View style={s.dotsRow}>
            {items.map((_, i) => {
              ...
              return <Animated.View key={i} style={[s.dot, { width: dotW, opacity }]} />;
            })}
          </View>
        </Animated.View>
      )}
...
function VitalsBanner() {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(heartScale, { toValue: 1.18, duration: 480, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        Animated.timing(heartScale, { toValue: 1.0, duration: 520, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
      ])
    ).start();
  }, []);

  // moving ECG bars (each bar with blue→green gradient)
  const BAR_COUNT = 28, BAR_MIN = 6, BAR_MAX = 30;
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      for (let i = 0; i < BAR_COUNT; i++) {
        const idx = ((i - offsetRef.current) % pattern.length + pattern.length) % pattern.length;
        const factor = pattern[idx];
        const target = BAR_MIN + factor * (BAR_MAX - BAR_MIN);
        Animated.timing(bars[i], { toValue: target, duration: 110, useNativeDriver: false }).start();
      }
    }, 110);
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(fill, { toValue: 1, duration: 1800, easing: Easing.inOut(Easing.quad), useNativeDriver: false }),
        Animated.timing(fill, { toValue: 0, duration: 1200, easing: Easing.out(Easing.quad), useNativeDriver: false }),
      ])
    ).start();
  }, []);
  const tubeFillH = fill.interpolate({ inputRange: [0, 1], outputRange: [4, 42] });
...
function SeverityBadge({ level }) {
  ...
  const haloOpacity = pulse.interpolate({ inputRange: [0, 1], outputRange: [0.0, 0.35] });

  return (
    <View style={{ alignItems: "center", justifyContent: "center" }}>
      {level === "high" && (
        <Animated.View
          style={[s.badgeHalo, { transform: [{ scale: haloScale }], opacity: haloOpacity, backgroundColor: c.bg }]}
        />
      )}
      <View style={[s.badge, { backgroundColor: c.bg }]}>
        <Text style={[s.badgeTxt, { color: c.text }]}>{(level || "info").toUpperCase()}</Text>
      </View>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  headerRow: { marginTop: 46, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  headerTitle: { fontSize: 24, fontWeight: "900", color: "#FFFFFF", lineHeight: 35 },
mediSlotTitle: { fontSize: 28, fontWeight: "900", color: "#FFFFFF" },
  ...
  barWrap: { width: 6, alignItems: "center", justifyContent: "flex-end", height: 36 },
  bar: { width: 4, borderRadius: 2, overflow: "hidden" },

  tube: {
    width: 22, height: 46, borderRadius: 6,
  ...
  iconWrap: {
    width: 28, height: 28, alignItems: "center", justifyContent: "center", position: "relative",
  },
  badge: {
    position: "absolute",
    right: -4, top: -4, minWidth: 18, height: 18, paddingHorizontal: 4,
  ...
  card: {
    ...
    backgroundColor: "#FFFFFF", marginRight: 16, borderWidth: 1, borderColor: C.border, elevation: 2,
  },
  cardImgTop: { width: "100%", height: 150, backgroundColor: "#EEF2FF" },
  cardImgPlaceholder: { alignItems: "center", justifyContent: "center" },
  cardBody: { padding: 12 },
  cardTitleRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  cardTitle: { color: C.text, fontWeight: "900", fontSize: 16, flex: 1, marginRight: 8 },
  cardSummary: { color: "#334155", marginTop: 6 },
  pillsRow: { flexDirection: "row", gap: 8, marginTop: 10 },

  pill: { backgroundColor: "#EEF2FF", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 },
